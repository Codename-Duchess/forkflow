...
export function AvatarImage({ src, alt }: { src: string; alt: string }) {
    return src ? <Image src={src} alt={alt} className="w-full h-full object-cover rounded-full" /> : null
}

export function AvatarFallback({ children }: React.PropsWithChildren<{}>) { // eslint-disable-line @typescript-eslint/no-empty-object-type
